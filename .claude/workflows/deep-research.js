...
const quotedLabel = (s) => {
  ...
  return (
    '"' + cps.slice(0, LABEL_CAP).join('').trim() + (cps.length > LABEL_CAP ? '\u2026' : '') + '"'
  );
};
const seen = new Map();
const dupes = [];
...
if (confirmed.length === 0) {
  ...
  if (killed.length === 0 && unverified.length > 0) {
    summary =
      ...
      unverified.length +
      ' verifier panels failed (likely rate-limiting or API errors). This is an infrastructure failure, not a research finding. Raw extracted claims returned below; retry or verify manually.';
